...
class _PinPlusKeyBoardPackageState extends State<PinPlusKeyBoardPackage> {
  ...
  Widget keyboardButtons(String number) {
    ...
    return Expanded(
      child: Padding(
        padding: EdgeInsets.symmetric(vertical: widget.keyboardVerticalSpacing, horizontal: _sizeW * 0.01),
        child: GestureDetector(
          ...
          child: Container(
            ...
            width: widget.keyboardBtnSize ?? (widget.keyboardButtonShape == KeyboardButtonShape.circular ? _sizeW * 0.13 : _sizeW * 0.1),
            height: widget.keyboardBtnSize ?? (widget.keyboardButtonShape == KeyboardButtonShape.circular ? _sizeW * 0.13 : _sizeW * 0.1),
            decoration: BoxDecoration(
                ...
                borderRadius: widget.keyoardBtnBorderRadius ??
                    (widget.keyboardButtonShape == KeyboardButtonShape.rounded ? BorderRadius.circular(_sizeW) : null),
                boxShadow: [
                  BoxShadow(
                    ...
                    spreadRadius: 3,
                    blurRadius: 9,
                    offset: Offset(0, widget.btnElevation ?? 0), // changes position of shadow
                  ),
                ],
                shape: widget.keyboardButtonShape == KeyboardButtonShape.circular ? BoxShape.circle : BoxShape.rectangle),
            child: Text(
              number,
              textAlign: TextAlign.center,
              style: TextStyle(
                  fontFamily: widget.keyboardFontFamily ?? Theme.of(context).textTheme.titleMedium?.fontFamily,
  ...
  Widget inputWidget(int position) {
    try {
      return Container(
        height: widget.inputHeight ?? widget.inputWidth ?? MediaQuery.of(context).size.width * 0.1,
        width: widget.inputWidth ?? MediaQuery.of(context).size.width * 0.1,
        decoration: BoxDecoration(
            border: widget.inputType == InputType.dash
                ? Border(
                    bottom: BorderSide(
                        color: widget.focusColor ?? widget.inputBorderColor ?? Colors.black, width: widget.inputBorderThickness ?? 0))
                : widget.inputHasBorder == false
                    ? null
                    : Border.all(
                        color: widget.focusColor ?? widget.inputBorderColor ?? Colors.black, width: widget.inputBorderThickness ?? 0),
            color: widget.isInputHidden == true ? widget.inputHiddenColor : Colors.transparent,
            borderRadius:
                widget.inputBorderRadius ?? (widget.inputShape == InputShape.rounded ? const BorderRadius.all(Radius.circular(100)) : null),
            boxShadow: [
              BoxShadow(
                color: widget.inputElevation == null
                    ? Colors.transparent
                    : widget.inputShadowColor?.withOpacity(0.6) ?? widget.inputFillColor?.withOpacity(0.6) ?? Colors.black.withOpacity(0.6),
                spreadRadius: 3,
                blurRadius: 9,
                offset: Offset(0, widget.inputElevation ?? 0), // changes position of shadow
              ),
            ],
            shape: widget.inputShape == InputShape.circular ? BoxShape.circle : BoxShape.rectangle),
        child: Center(
            child: Text(
          res[position],
          ...
          style: widget.inputTextStyle ??
              TextStyle(
                color: widget.isInputHidden == true ? widget.inputHiddenColor : widget.inputTextColor ?? Colors.black,
              ),
        )),
      );
    } catch (e) {
      return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: widget.inputType == InputType.dash
                ? Border(bottom: BorderSide(color: widget.inputBorderColor ?? Colors.black, width: widget.inputBorderThickness ?? 0))
                : widget.inputHasBorder == false
                    ? null
                    : Border.all(color: widget.inputBorderColor ?? Colors.black, width: widget.inputBorderThickness ?? 0),
            borderRadius:
                widget.inputBorderRadius ?? (widget.inputShape == InputShape.rounded ? const BorderRadius.all(Radius.circular(100)) : null),
            boxShadow: [
              BoxShadow(
                  ...
                  (widget.extraInput == null
                      ? Expanded(
                          ...
                          icon: widget.doneButton ??
                              Icon(
                                Icons.done,
                                color: widget.inputFillColor ?? widget.inputBorderColor ?? Colors.black,
                              ),
                        ))
